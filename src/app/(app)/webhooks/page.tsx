...
export default async function WebhooksPage() {
  ...
  return (
    <div>
      ...
      <Card className="p-4 mb-6">
        <h2 className="text-sm font-semibold text-foreground-950 mb-1">2. What happens on each order</h2>
        <p className="text-xs text-foreground-500 max-w-2xl">
          The plugin posts <span className="font-mono">order.created</span> / <span className="font-mono">order.updated</span> webhooks,
          signed with a per-brand secret, to this endpoint. It upserts the contact, calculates COGS, merchant fee,
          affiliate commission, and net profit, then decrements master stock — all in one atomic transaction.
        </p>
      </Card>

      <div>
        <h2 className="text-sm font-semibold text-foreground-950 mb-3">Connected brands</h2>
        {brands.length === 0 ? (
          <p className="text-sm text-foreground-500">
            No brands yet — install the plugin on your first WooCommerce site to see it appear here.
          </p>
        ) : (
